...
const getGames = async() => {
    try {
        const resp = await fetch(`https://api.rawg.io/api/games?key=6f2b18a8c113418299c8b7a82d268af9&page=${pageCounter}`)
...
let observer = new IntersectionObserver((gameObserved) => {
		...
		if(gameObserved.isIntersecting && !Search){
			pageCounter++;
			getGames();
		}
	});
       ...
       gamesCards += `
        ...
        <div class = "col-4 py-2">
            <div class="card">
                ...
                <img class="card-img" id = "gameFrontPage" src="${game.background_image}">
...
/* begin getGen -> obtains the genres of the games */
function getGen(genres){
    let genresCards = ''
    for (let index = 0; index < genres.length; index++) {
        genresCards += genres[index].name;
        if (index + 1 != genres.length) {  // If there are more than one genre.
            genresCards += ', '
        }
        ...
        const element = platforms[index];
            ...
            switch  (element.platform.id){
                case (2): // Play
                    platformCards += `
                        <div class = "col-4">
                            <svg  id ="PS" width="24" height="20" viewBox="0 0 24 20" fill="#FFFFFF" xmlns="http://www.w3.org/2000/svg">
                ...
                case (3): // Xbox
                    ...
                    platformCards += `
                        <div class = "col-4">
                            <svg  id="Xbox" width="20" height="20" viewBox="0 0 20 20" fill="#FFFFFF" xmlns="http://www.w3.org/2000/svg">
                ...
                case (7): // Nintendo
                    ...
                    platformCards += `
                        <div class = "col-4">
                            <svg id="Nintendo" width="24" height="24" viewBox="0 0 24 24" fill="#FFFFFF" xmlns="http://www.w3.org/2000/svg">
                ...
                case (1): // PC
                    ...
                    platformCards += `
                        <div class = "col-4">
                            <svg id ="PC" width="20" height="20" viewBox="0 0 20 20" fill="#FFFFFF" xmlns="http://www.w3.org/2000/svg">
...
searchTermElem.addEventListener('input', function (event) {
    ...
    const showResults = searchTerm => {
        ...
        if (resultsCounter > 4) resultsCounter = 4
    ...
    const search = debounce(async (searchTerm) => {
        ...
        try {
            const response = await fetch(`https://api.rawg.io/api/games?key=6f2b18a8c113418299c8b7a82d268af9&search=${searchTerm}`);
            const searchResults = await response.json()
            gamesCards = ''
            id = 1
            resultsToShow = searchResults.results
            ...
            if (resultsToShow.length == 0){
                ...
                resultsCounter = resultsToShow.length 
                getData(resultsToShow.slice(0,4))
